...
export default class CartScreen extends Component {
  ...
  async updateCartItemsPurcahseState(transaction) {
    var currentUser = Parse.User.current();
    if (currentUser) {
      ...
      <StyleProvider style={getTheme(material)}>
        <Container style={{ flex: 1 }}>
          {this.state.fetching ? (
            ...
            <React.Fragment>
              <Header style={[styles.bgLeafGreen, { paddingVertical: 10, justifyContent: "center" }]}>
                <Left>
                ...
                <Body>
                  <Text style={[styles.whiteText, { fontSize: 18 }]}>Cart Items</Text>
                </Body>
                <Right>
                  {this.state.cartItems.length > 0 && !this.state.purchaseSuccess ?
                    <Button transparent iconRight onPress={() => this.setState({ showModal: true })}>
                      <Text>Checkout&nbsp;</Text>
                      <Icon icon name={"ios-checkmark-circle-outline"} style={styles.whiteText} />
                    </Button>
                    : null}
                </Right>
              </Header>
